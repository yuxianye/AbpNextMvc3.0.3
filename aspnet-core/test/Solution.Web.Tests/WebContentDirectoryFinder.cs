...
namespace Solution
{
    ...
    public static class WebContentDirectoryFinder
    {
        ...
        {
            var domainAssemblyDirectoryPath = Path.GetDirectoryName(typeof(SolutionDomainModule).Assembly.Location);
            if (domainAssemblyDirectoryPath == null)
            {
                throw new Exception($"Could not find location of {typeof(SolutionDomainModule).Assembly.FullName} assembly!");
            }
            ...
            var webFolder = Path.Combine(directoryInfo.FullName, $"src{Path.DirectorySeparatorChar}Solution.Web");
